...
export default function Seven() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setData(res.data);
        setExpandedItems(new Array(res.data.length).fill(false));
      })
      .catch((err) => console.log(err));
  }, []);

  const toggleItem = (index) => {
    // Créez une copie du tableau expandedItems
    const newExpandedItems = [...expandedItems];
    // Inversez la valeur pour l'élément cliqué
        ...
        <div className="flex justify-center items-center flex-col mt-16 sm:mt-32 space-y-2">
          ...
          <div className="flex flex-col space-y-12 font-thin w-full justify-center items-center sm:px-0 px-8">
            {data.map((el, index) => (
              <div
                key={index}
                className="flex flex-col sm:w-[60%]  text-white py-4 px-3 bg-[#404040] rounded-lg"
              >
                ...
                {expandedItems[index] && (
                  <div className="mt-4 text-white flex flex-col">
                    <p className="text-white text-base">{el.description}</p>
                    <a href={el.go_to}>
